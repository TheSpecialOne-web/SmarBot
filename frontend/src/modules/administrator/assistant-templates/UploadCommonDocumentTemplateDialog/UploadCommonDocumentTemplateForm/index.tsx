...
import {
  FileValidationType,
  validateFileForCommonDocument,
  validateFileForCommonDocumentV2,
} from "@/libs/administrator/files";
import {
...
type Props = {
  onSubmit: (uplaod: CreateCommonDocumentTemplateParam) => Promise<void>;
  onClose: () => void;
  documents: CommonDocumentTemplate[];
};

export const UploadCommonDocumentTemplateForm = ({ onSubmit, onClose, documents }: Props) => {
  ...
  const {
    ...
    setValue,
    clearErrors,
  } = useForm<CreateCommonDocumentTemplateParam>({
    defaultValues: {
      memo: "",
      files: [],
    },
    ...
    (newFiles: File[]) => {
      ...
      for (const file of newFiles) {
        ...
        legacyMicrosoftOfficeExtension
          ? (validateError = validateFileForCommonDocumentV2(file, validFiles, documents))
          : (validateError = validateFileForCommonDocument(file, validFiles, documents));
    ...
    <form onSubmit={handleSubmit(onSubmit)} noValidate>
      <CustomDialogContent>
        ...
        >
          ...
          <Typography fontSize={12}>または</Typography>
          <input {...getInputProps()} />
          <MultipleFileInput
            allowed_extension={
              legacyMicrosoftOfficeExtension
        ...
        <Typography variant="caption">※最大10ファイルまで一度にアップロードできます。</Typography>
      </CustomDialogContent>
