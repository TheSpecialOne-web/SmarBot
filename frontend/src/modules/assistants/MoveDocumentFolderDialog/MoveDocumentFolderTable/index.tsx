...
type Props = {
  ...
  displayedDocumentFolderId: string;
  documents: Document[];
  displayedDocumentFolders: DocumentFolder[];
  displayedDocumentFolderDetail?: DocumentFolderDetail;
  currentParentDocumentFolderDetail: DocumentFolderDetail;
  onMoveToFolder: (folderId: string) => void;
  onSubmit: (param: MoveDocumentFolderParam) => void;
  onClose: () => void;
  isLoading: boolean;
};
...
export const MoveDocumentFolderTable = ({
  targetDocumentFolder,
  rootDocumentFolder,
  displayedDocumentFolderId,
  documents,
  displayedDocumentFolders,
  ...
  const tableColumns: CustomTableColumn<TableRow>[] = [
    {
      ...
      render: useCallback(
        (row: TableRow) => {
          ...
          return (
            <Stack direction="row" gap={1} alignItems="center">
              ...
              {row.id === targetDocumentFolder.id ? (
        ...
        [onMoveToFolder, targetDocumentFolder.id],
      ),
  ...
  const tableData: TableRow[] = [
    ...displayedDocumentFolders.map(documentFolder => ({
      ...createNullifiedDocument(),
    ...
    return (
      <Stack direction="row" gap={1} alignContent="center">
        <Button
          variant="outlined"
          color="primary"
          onClick={() => onSubmit({ new_parent_document_folder_id: row.id })}
          disabled={
            row.id === targetDocumentFolder.id || row.id === currentParentDocumentFolderDetail?.id
          }
        >
          移動
        </Button>
        <IconButton
          ...
          disabled={row.id === targetDocumentFolder.id}
        >
  ...
  if (isLoading || !displayedDocumentFolderId) {
    return (
      <Container sx={{ py: 4 }}>
        <CustomTableSkeleton />
      </Container>
  ...
  return (
    <>
      ...
      <CustomDialogAction
        onSubmit={() => onSubmit({ new_parent_document_folder_id: displayedDocumentFolderId })}
        onClose={onClose}
        buttonText="移動"
        disabled={
          !displayedDocumentFolderId ||
          currentParentDocumentFolderDetail?.id === displayedDocumentFolderId
        }
      />
    </>
  );
