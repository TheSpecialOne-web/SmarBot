...
import { useMoveDocument } from "@/orval/backend-api";
import { Bot, Document, DocumentFolderDetail, MoveDocumentParam } from "@/orval/models/backend-api";
...
export const ConfirmMoveDocumentDialog = ({
  bot,
  open,
  targetDocument,
  ...
  const { trigger, isMutating } = useMoveDocument(bot.id, targetDocument.id);

  const handleMoveDocumentFolder = async (param: MoveDocumentParam) => {
    try {
      await trigger(param);
      refetch();
      onCloseConfirmDialog();
    ...
    <ConfirmDialog
      ...
      onSubmit={() =>
        handleMoveDocumentFolder({
          document_folder_id: newParentDocumentFolderDetail.id,
        })
      }
