...
class Table(BaseModel):
    ...
    def drop_non_header_rows(self, row_idx_start: int, row_idx_end: int, max_header_row_count: int = 3) -> None:
        undroppable_header_row_idxs = [
            _row_idx for _row_idx in range(row_idx_start, row_idx_end) if _row_idx in self.header_row_idxs
        ]
    ...
    def to_text(self, row_idx_start: int = 0, row_idx_end: int | None = None) -> str:
        ...
        table = Table(
            matrix=deepcopy(self.matrix[0:row_idx_end]), caption=self.caption, header_row_idxs=self.header_row_idxs
        )
    ...
    @classmethod
    def from_document_intelligence_table(cls, table_di: FormRecognizerTable) -> "Table":
        if table_di.row_count == 0 or table_di.column_count == 0:
    ...
    def from_docx_table(cls, table_docx: DocxTable) -> "Table":
        if len(table_docx.columns) == 0 or len(table_docx.rows) == 0:
