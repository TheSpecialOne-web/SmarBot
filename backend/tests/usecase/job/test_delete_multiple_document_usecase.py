...
from api.usecase.job.delete_multiple_documents import DeleteMultipleDocumentsUseCase


class TestJobDeleteMultipleDocumentsUseCase:
    @pytest.fixture
    def setup(self):
        ...
        self.use_case = DeleteMultipleDocumentsUseCase(
        ...
        self.document_ids: list[document_domain.Id] = [document_domain.Id(value=1)]
        ...
        self.cognitive_search_service_mock.delete_documents_from_index_by_document_id.return_value = None
        self.blob_storage_service_mock.delete_document_blob_v2.return_value = None

    @pytest.fixture
    def mock_get_feature_flag(self, monkeypatch):
        mock_get_feature_flag = Mock()
        monkeypatch.setattr("api.usecase.job.delete_multiple_documents.get_feature_flag", mock_get_feature_flag)
        return mock_get_feature_flag

    def test_delete_multiple_documents_success_without_feature_flag(self, setup, mock_get_feature_flag):
        mock_get_feature_flag.return_value = False
        # Call the method
        self.use_case.delete_multiple_documents(self.tenant_id, self.bot_id, self.document_ids)
        # Assertions
        ...
        self.cognitive_search_service_mock.delete_documents_from_index_by_document_id.assert_called_once_with(
            endpoint=self.search_service_endpoint, index_name=self.index_name, document_id=self.document.id
        )
        self.document_repo_mock.delete.assert_called_once_with(self.document.id)
        self.blob_storage_service_mock.delete_document_blob.assert_called_once_with(
    ...
    def test_delete_multiple_documents_success_with_feature_flag(self, setup, mock_get_feature_flag):
        ...
        self.document_repo_mock.delete.assert_called_once_with(self.document.id)
        self.blob_storage_service_mock.delete_document_blob_v2.assert_called_once_with(
